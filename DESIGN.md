...
Furthermore, to implement the notification mechanism, I implemented a named FIFO for each process, which is recorded in the table. In those FIFOs, opened in RDWR (to avoid them being closed when the last writer disconnects), other clients notify the client when there is a request of transfering content.
When a client sends a request to another client, and that other client is not online, the sender remains in a "pending state", waiting for the receiver to reconnect. The sender, upon login, will check if there are any pending requests.
